<x-app-layout>
    ...
    <div class="max-w-5xl w-full mx-auto px-4 py-4 sm:py-6">
        <div class="max-w-3xl mx-auto border rounded-2xl px-4 sm:px-8 py-6 shadow bg-white dark:bg-gray-800 space-y-4 flex justify-center">
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <a href="{{ route('admin.artists.edit', $artist) }}" class="inline-block">
                                    <x-icons.pencil-square size="size-4" />
                                </a>
                            </td>
                            <td class="text-center">
                                <form method="POST" action="{{ route('admin.artists.destroy', $artist) }}" onsubmit="return confirm('削除してよろしいですか？')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"><x-icons.trash size="size-4" /></button>
                                </form>
                            </td>
                        </tr>
                        @endforeach
