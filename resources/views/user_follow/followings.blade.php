<x-app-layout>
    <x-slot name="title">Oshi Graphy | フォロー一覧</x-slot>

    <x-slot name="header">
        <div class="max-w-3xl mx-auto flex items-center gap-10 justify-center">
            <h2 class="text-base font-semibold text-brand-dark">フォロー一覧</h2>
        </div>
    </x-slot>
    ...
    <nav class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mt-3 sm:mt-5 flex justify-between items-center text-[11px] text-gray-600 dark:text-gray-300 sm:text-base no-print">
        <div>
            ...
            <span>フォロー一覧({{ $followings?->total() ?? 0 }}人)</span>
        </div>
    ...
    <div class="motion-safe:animate-fade-up">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <ul class="space-y-4">
                ...
                @empty
                <li class="text-base text-gray-500 ml-4">まだ誰もフォローしていません。</li>
                @endforelse
            </ul>
            <div class="mt-6">{{ $followings->links() }}</div>
        </div>
    </div>
</x-app-layout>